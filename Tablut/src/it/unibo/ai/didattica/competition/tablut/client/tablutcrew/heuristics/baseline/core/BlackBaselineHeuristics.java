...
/**
 * Black player composite heuristic that combines multiple heuristics
 * based on provided weights configuration.
 */
public class BlackBaselineHeuristics extends Heuristic {
     ...
     */
    public BlackBaselineHeuristics(State state, State.Turn currentPlayer, Map<String, Double> weights) {
        ...
        if (weights == null || weights.isEmpty()) {
            this.weights = BlackWeights.getDefaultWeights();
        } else {
     ...
     */
    public BlackBaselineHeuristics(State state, State.Turn currentPlayer) {
        this(state, currentPlayer, BlackWeights.getDefaultWeights());
    }
    ...
    private Map<String, Double> filterValidWeights(Map<String, Double> providedWeights) {
        ...
        for (Map.Entry<String, Double> entry : providedWeights.entrySet()) {
            ...
            if (BlackWeights.getAllKeys().contains(key)) {
    ...
    public double evaluateState(State state) {
        ...
        for (Map.Entry<String, Double> entry : weights.entrySet()) {
            ...
            if (HeuristicRegistry.hasBlackHeuristic(key)) {
                try {
